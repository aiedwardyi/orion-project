...
const LookupModal: React.FC<{ onClose: () => void; isDarkMode: boolean }> = ({ onClose, isDarkMode }) => {
  ...
  const recentSearches = ['LH190', 'Pyongyang Hotel', 'BA234', 'Koryo'];
        ...
        <div className="p-6 space-y-6">
          ...
          {query.length > 0 && (
            <div className="space-y-2 pt-2 animate-in slide-in-from-top-2 duration-300">
               ...
               <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-white/[0.02] border-white/5' : 'bg-zinc-50 border-zinc-200'} flex justify-between items-center group cursor-pointer hover:border-amber-500/40 transition-all`}>
                  <div>
...
const DigitalTicketModal: React.FC<{ flight: Flight; onClose: () => void; isDarkMode: boolean }> = ({ flight, onClose, isDarkMode }) => {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      <div className="w-full max-w-sm relative group animate-in zoom-in-95 duration-300">
...
const SeatingChartModal: React.FC<{ flight: Flight; onClose: () => void; isDarkMode: boolean }> = ({ flight, onClose, isDarkMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      <div className="w-full max-w-sm relative animate-in slide-in-from-bottom-8 duration-500">
        ...
        <GlassCard isDarkMode={isDarkMode} className="p-0 overflow-hidden border-amber-500/30 shadow-3xl">
          <div className={`p-6 border-b ${isDarkMode ? 'border-white/10 bg-white/[0.02]' : 'border-zinc-100 bg-zinc-50'} flex justify-between items-center`}>
            <div>
              ...
              <p className="text-[9px] text-amber-500 font-black uppercase tracking-[0.2em] mt-0.5">ELITE TIER OPTIONS</p>
            </div>
            <button onClick={onClose} className="w-10 h-10 rounded-2xl flex items-center justify-center bg-white/5 text-zinc-500 hover:text-amber-500 transition-all">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          ...
          <div className="p-6 space-y-3">
            {upgrades.map(u => (
              <button key={u.title} onClick={onClose} className={`w-full p-4 rounded-2xl border text-left transition-all active:scale-[0.98] ${isDarkMode ? 'bg-white/[0.02] border-white/5 hover:border-amber-500/40' : 'bg-zinc-50 border-zinc-200 hover:border-amber-500'}`}>
      ...
      <div className="w-full max-w-sm relative animate-in slide-in-from-bottom-8 duration-500">
        <GlassCard isDarkMode={isDarkMode} className="p-0 overflow-hidden border-amber-500/30 shadow-3xl">
          ...
          <div className="p-6 grid grid-cols-2 gap-3">
             {treatments.map(t => (
               <button key={t.name} onClick={onClose} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 text-center transition-all active:scale-95 ${isDarkMode ? 'bg-white/[0.02] border-white/5 hover:bg-amber-500/5 hover:border-amber-500/30' : 'bg-zinc-50 border-zinc-200 hover:bg-amber-50'}`}>
...
const Travel: React.FC<TravelProps> = ({ isDarkMode, initialTab = 'flights' }) => {
  ...
  const hotelImages = [
    "https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?q=80&w=1600&auto=format&fit=crop", // Iconic Ryugyong Style Grandeur
    "https://images.unsplash.com/photo-1566665797739-1674de7a421a?q=80&w=1600&auto=format&fit=crop"  // Paddock Guest Suites Style
  ];
            ...
            <GlassCard isDarkMode={isDarkMode} key={flight.id} noPadding className={`border-white/10 transition-all duration-500 ${expandedItem === flight.id ? 'shadow-[0_0_30px_rgba(245,158,11,0.1)] ring-1 ring-amber-500/20' : ''}`}>
               ...
               <div className="p-5 flex flex-col gap-6">
                 ...
                 <div className="grid grid-cols-2 gap-2">
                    ...
                    <button className="bg-amber-500 hover:bg-amber-400 hover:brightness-110 text-black py-3 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-amber-500/20 active:scale-95 transition-all duration-300">
                      Confirm Travel
                    </button>
                 </div>
               </div>
              ...
              >
                ...
                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                   <div className="space-y-1">
                      ...
                      <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-0.5">{idx === 0 ? 'ICONIC RYUGYONG' : 'KORYO TWIN TOWERS'}</p>
