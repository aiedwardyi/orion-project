...
const LiveStreamPopup: React.FC<{ isOpen: boolean; onClose: () => void; isDarkMode: boolean }> = ({ isOpen, onClose, isDarkMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-500">
      ...
      <GlassCard isDarkMode={isDarkMode} noPadding className="w-full max-w-2xl aspect-video border-amber-500/40 shadow-[0_0_80px_rgba(245,158,11,0.25)] overflow-hidden relative z-10">
        {/* Video Background */}
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          {/* Scanning Line */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-amber-500/30 blur-sm scanning-beam"></div>
          
          {/* Grid Lines */}
          <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle, #f59e0b 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
          ...
          <div className="absolute top-6 left-6 right-6 flex justify-between items-start">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-white uppercase tracking-[0.3em] drop-shadow-md">LIVE SIGNAL • OIS-SAT-04</span>
              </div>
              <h3 className="text-xl font-black italic tracking-tighter text-white uppercase drop-shadow-lg">Pyongyang Cup: Finals</h3>
            </div>
            <button 
              onClick={onClose}
              className="w-10 h-10 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-center text-white hover:bg-amber-500 hover:text-black transition-all pointer-events-auto active:scale-90"
            >
          ...
          <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
             <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10">
                <p className="text-[8px] text-zinc-500 font-black uppercase tracking-widest mb-0.5">Telemetry Data</p>
                <div className="flex gap-4">
                   <div className="flex flex-col">
                      <span className="text-[7px] text-amber-500 font-black uppercase">Signal</span>
                      <span className="text-[10px] font-black text-white mono">4.2 TB/S</span>
                   </div>
                   <div className="flex flex-col border-l border-white/10 pl-4">
                      <span className="text-[7px] text-amber-500 font-black uppercase">Latency</span>
                      <span className="text-[10px] font-black text-white mono">12MS</span>
                   </div>
                </div>
             </div>
             <div className="text-right">
                <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest italic drop-shadow-md">Grand Arena • Sector VIP</p>
                <p className="text-[8px] text-white/40 font-bold uppercase tracking-[0.4em] mt-1">Direct Encrypted Stream</p>
             </div>
          </div>
        </div>
      </GlassCard>
...
const NotificationPopup: React.FC<{ isOpen: boolean; onClose: () => void; isDarkMode: boolean }> = ({ isOpen, onClose, isDarkMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-start justify-center p-6 pt-20 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <GlassCard isDarkMode={isDarkMode} className="w-full max-sm border-amber-500/30 shadow-[0_30px_60px_-12px_rgba(0,0,0,0.8)] p-0 overflow-hidden relative z-10">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3.5">
          <button 
            ...
            className={`w-11 h-11 rounded-2xl border p-1 ring-4 shadow-2xl overflow-hidden flex items-center justify-center active:scale-95 transition-all ${isDarkMode ? 'bg-amber-500/10 border-amber-500/30 ring-black' : 'bg-amber-100 border-amber-200 ring-white'}`}
          >
             <img 
               src="https://aiedwardyi.s3.ap-northeast-2.amazonaws.com/wonikyi.jpg" 
               alt="Edward Yi Avatar" 
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6">
            <div className="flex justify-between items-end gap-10">
              ...
              <div 
                className="bg-amber-500 w-12 h-12 shrink-0 rounded-2xl flex items-center justify-center text-black shadow-lg shadow-amber-500/30 active:scale-90 transition-all cursor-pointer group-hover:brightness-110"
              >
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5l7 7-7 7"/></svg>
              </div>
            </div>
          </div>
        </GlassCard>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="space-y-0.5">
            ...
            <p className={`text-xs font-bold uppercase italic ${isDarkMode ? 'text-white' : 'text-zinc-800'}`}>AA123 • Gate 4B</p>
          </div>
        </GlassCard>
        ...
        >
          ...
          <div className="space-y-0.5">
            ...
            <p className={`text-xs font-bold uppercase italic ${isDarkMode ? 'text-white' : 'text-zinc-800'}`}>Yanggakdo • R1043</p>
          </div>
        </GlassCard>
      </div>
          ...
          {MOCK_SCHEDULE.slice(1, 4).map(item => (
            <GlassCard isDarkMode={isDarkMode} key={item.id} className="flex items-center gap-4 py-4 px-5 border-white/[0.04] active:bg-white/[0.02] shadow-sm">
              ...
              <div className={`${isDarkMode ? 'text-zinc-700' : 'text-zinc-300'}`}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5l7 7-7 7"/></svg>
              </div>
            </GlassCard>
          ))}
        </div>
