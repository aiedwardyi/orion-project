...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    switch (currentScreen) {
      ...
      case 'Travel':
      case 'Profile':
        overlay.classList.add('tint-amber');
        break;
      case 'MyDay':
      case 'Requests':
        overlay.classList.add('tint-green');
        break;
      default:
        overlay.classList.add('tint-neutral');
    }
  ...
  const handleNavigateToTravel = (tab: 'flights' | 'hotels') => {
    setTravelTab(tab);
    ...
    switch (currentScreen) {
      case 'Home': return (
        <Home 
          ...
          onNavigateToTravel={handleNavigateToTravel} 
        />
      );
      case 'MyDay': return <MyDay isDarkMode={isDarkMode} />;
      case 'Requests': return <Requests isDarkMode={isDarkMode} />;
      case 'Travel': return <Travel isDarkMode={isDarkMode} initialTab={travelTab} />;
      case 'Profile': return (
          ...
          <div className="w-full space-y-4">
            <div className="relative group">
              ...
              <GlassCard isDarkMode={isDarkMode} className={`relative p-0 overflow-hidden ${isDarkMode ? 'border-amber-500/30 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.6)]' : 'border-zinc-200 shadow-lg'}`}>
                <div className="absolute inset-0 opacity-[0.02] pointer-events-none mix-blend-screen" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, white 1px, transparent 0)', backgroundSize: '24px 24px' }}></div>
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="relative">
                      ...
                      <div className={`w-16 h-16 rounded-2xl border-2 border-amber-500/40 p-1 flex items-center justify-center relative z-10 overflow-hidden shadow-2xl ${isDarkMode ? 'bg-neutral-900' : 'bg-zinc-50'}`}>
                        <span className={`text-xl font-black italic tracking-tighter leading-none pr-1 mt-0.5 ${isDarkMode ? 'text-amber-500' : 'text-amber-600'}`}>EY</span>
                      </div>
                    </div>
              ...
              <GlassCard isDarkMode={isDarkMode} className={`p-4 ${isDarkMode ? 'border-amber-500/10 bg-gradient-to-b from-neutral-900/40 to-black/60 shadow-xl' : 'bg-white shadow-md border-zinc-200'} flex flex-col items-center justify-center gap-4`}>
                <div className={`relative w-[180px] h-[180px] flex items-center justify-center rounded-[1.5rem] border mx-auto group cursor-none ${isDarkMode ? 'bg-black/60 border-white/5' : 'bg-zinc-100 border-zinc-200'}`}>
                  ...
                  <div className={`bg-black rounded-lg overflow-hidden flex items-center justify-center w-[130px] h-[130px] shadow-2xl relative z-10 transition-transform duration-700 hover:scale-105`}>
                    <img 
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=130x130&data=ORION-AUTH-LINK-V9823-SYNC-${Date.now()}&bgcolor=000&color=f59e0b&margin=0`} 
                      alt="Encrypted Pass" 
                      className="w-[120px] h-[120px] object-contain opacity-90 brightness-110"
                    />
                  </div>
                </div>
            ...
            <div className="space-y-4">
                ...
                <GlassCard isDarkMode={isDarkMode} className={`p-4 ${isDarkMode ? 'border-white/5' : 'border-zinc-200 shadow-sm'}`}>
                  <div className="space-y-3">
                    ...
                    <button 
                      ...
                      className={`w-full py-3 border-2 border-amber-500/50 rounded-xl text-[8px] font-black uppercase tracking-[0.3em] transition-all active:scale-[0.98] ${isDarkMode ? 'text-amber-500 hover:bg-amber-500/10' : 'text-amber-600 hover:bg-amber-50'}`}>
              ...
              <button 
                ...
                className={`w-full py-6 font-black text-[8px] transition-all uppercase tracking-[0.6em] active:scale-95 ${isDarkMode ? 'text-zinc-800 hover:text-red-500/60' : 'text-zinc-300 hover:text-red-600/60'}`}
              >
      ...
      default: return <Home isDarkMode={isDarkMode} onToggleTheme={toggleTheme} onNavigateToTravel={handleNavigateToTravel} />;
    }
  };

  return (
    <div className={`min-h-screen transition-colors duration-700 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>
      <main className="max-w-2xl mx-auto min-h-screen pb-32">
        {renderScreen()}
      </main>
          ...
          <div className="flex justify-between items-center px-2">
            ...
            <NavItem isDarkMode={isDarkMode} active={currentScreen === 'Profile'} onClick={() => setCurrentScreen('Profile')} label="Pass" icon={
              <div className={`w-6 h-6 rounded-full overflow-hidden border-2 transition-all duration-300 flex items-center justify-center ${currentScreen === 'Profile' ? 'border-amber-500 scale-110 shadow-[0_0_8px_rgba(245,158,11,0.6)]' : 'border-zinc-300 grayscale opacity-60'}`}>
                <span className={`text-[7px] font-black italic tracking-tighter leading-none pr-[0.5px] mt-[0.5px] ${isDarkMode ? 'text-amber-500' : 'text-amber-600'}`}>EY</span>
              </div>
            } />
          </div>
        </nav>
...
const NavItem: React.FC<{ active: boolean; label: string; icon: React.ReactNode; onClick: () => void; isDarkMode: boolean }> = ({ active, label, icon, onClick, isDarkMode }) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center justify-center py-2.5 px-3.5 transition-all rounded-full ${active ? 'bg-amber-500/10 text-amber-500' : 'text-zinc-400 active:scale-90'}`}
  >
